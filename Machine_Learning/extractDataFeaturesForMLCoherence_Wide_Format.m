function [data_coherence_table,data_phase_table] = extractDataFeaturesForMLCoherence_Wide_Format(files,subject_ID,stim_target,visit_number,varargin)
...
%     3;...                           % gRCS01
%     3;3];                           % gRCS02
% extractDataFeaturesForMLCoherence_Wide_Format(patient_files,patient_names,dbs_target,visit_number);
% extractDataFeaturesForMLCoherence_Wide_Format(patient_files,patient_names,dbs_target,visit_number,'power_type','peri','adj_time',0.05,'STFT_overlap',0.9);
% extractDataFeaturesForMLCoherence_Wide_Format(patient_files,patient_names,dbs_target,visit_number,'power_type','peri','adj_time',0.1,'STFT_overlap',0.9);
% extractDataFeaturesForMLCoherence_Wide_Format(patient_files,patient_names,dbs_target,visit_number,'power_type','pre','STFT_overlap',0.9);
...
if ~exist('spectral_type','var')
    spectral_type = 'wavelet';
end

if ~strcmp(spectral_type,'wavelet')
...
for i = 1:length(files)
    printStatus(sprintf('Loading file #%i: %s',i,files{i}));
    ...
    % Determine time adjustment given power type
...
%         adj_time(i) = calcAdjTime(aligned_data.gait_events,'inter-heel-strike');
    ...
    % Determine left and right contacts
    if isfield(aligned_data,'left_LFP_table')
        leftContactInds = find(contains(aligned_data.left_LFP_table.Properties.VariableNames,'key'));
        leftContactNames = aligned_data.left_LFP_table.Properties.VariableNames(leftContactInds);
    else
        leftContactInds = [];
        leftContactNames = [];
    end
    ...
    if isfield(aligned_data,'right_LFP_table')
        rightContactInds = find(contains(aligned_data.right_LFP_table.Properties.VariableNames,'key'));
        rightContactNames = aligned_data.right_LFP_table.Properties.VariableNames(rightContactInds);
    else
        rightContactInds = [];
        rightContactNames = [];
    end
    
    % Set up contact compare array. Compares same side to same side only
    % due to timing differences
    left_pairs = nchoosek(1:length(leftContactInds),2);
    right_pairs = nchoosek(1:length(rightContactInds),2);
    pairs = [left_pairs;right_pairs];
    
    % Loop through all pairs
    for j = 1:size(pairs,1)
        % Determine table indexing
        switch pairs(j,1)
            case 1
                contact1 = 'key0';
            case 2
        ...
        if j <= size(left_pairs,1)
            hemisphere = 'left_LFP_table';
        else
            hemisphere = 'right_LFP_table';
        end
        
        % Get sampline rate and set time vec
        if contains(hemisphere,'left')
        ...
        % Calculate coherence
        printStatus(sprintf('Extracting magnitude-squared coherence from %s %s %s',hemisphere,contact1,contact2));
        if strcmp(spectral_type,'wavelet')
            [wcoh,wcs,wfreq] = wcoherence(aligned_data.(hemisphere).(contact1),aligned_data.(hemisphere).(contact2),sr,'VoicesPerOctave',10);
        else
                % TODO fft coherence
        end
        
        % Extract features
        [freq_bin_inds,freq_vals_new] = genFreqBinPairs(wfreq,freq_limit);
        if ~isempty(freq_vals)
            if sum(abs(freq_vals-freq_vals_new) < 1e-05) ~= length(freq_vals)
        ...
        temp_coherence_data = nan(n_event_names*n_events,n_freq_bins);
        temp_phase_data = nan(n_event_names*n_events,n_freq_bins);
        for k = 1:n_event_names
            for m = 1:n_events
                if aligned_data.gait_events.(aligned_data.gait_events.Properties.VariableNames{k})(m) >= gait_event_range(1) && ...
                    ...
                    temp_coherence = nan(1,n_freq_bins);
                    temp_phase = nan(1,n_freq_bins);
                    event_time = aligned_data.gait_events.(aligned_data.gait_events.Properties.VariableNames{k})(m);
                    if ~isnan(event_time)
                        if strcmp(power_type,'instantaneous')
                            [~,event_ind] = min(abs(time_vec-event_time));
                        elseif strcmp(power_type,'peri')
                            [~,start_ind] = min(abs(time_vec-(event_time-adj_time(i))));
                            [~,end_ind] = min(abs(time_vec-(event_time+adj_time(i))));
                            event_ind = start_ind:end_ind;
                        elseif strcmp(power_type,'pre')
                            [~,start_ind] = min(abs(time_vec-(event_time-adj_time(i))));
                            [~,end_ind] = min(abs(time_vec-(event_time)));
                            event_ind = start_ind:end_ind;
                        end

                        coherence_snip = abs(wcoh(:,event_ind));
                        phase_snip = angle(wcs(:,event_ind));

                        if sum(isinf(coherence_snip),'all') == 0
                            parfor n = 1:n_freq_bins
                                mean_coherence = mean(coherence_snip(freq_bin_inds(n,1):freq_bin_inds(n,2),:),'all');
                                mean_phase = mean(phase_snip(freq_bin_inds(n,1):freq_bin_inds(n,2),:),'all');

                                if ~isinf(mean_coherence)
                                    temp_coherence(1,n) = mean_coherence;
                                    temp_phase(1,n) = mean_phase;
                                end
                            end
                        end

                        temp_coherence_data((k-1)*n_events+m,:) = temp_coherence;
                        temp_phase_data((k-1)*n_events+m,:) = temp_phase;
                    end
                end
            end
        end
        printStatus(sprintf('Complete extraction of magnitude-squared coherence from %s %s %s',hemisphere,contact1,contact2));
        
        data_coherence_mat = [data_coherence_mat;temp_coherence_data];
        data_phase_mat = [data_phase_mat;temp_phase_data];
        ids = [ids;repelem(subject_ID(i),size(temp_coherence_data,1),1)];
        targets = [targets;repelem(stim_target(i),size(temp_coherence_data,1),1)];
        if contains(hemisphere,'left')
            side = [side;repelem({'L'},size(temp_coherence_data,1),1)];
        else
            side = [side;repelem({'R'},size(temp_coherence_data,1),1)];
        end
        pair = [pair;repelem({sprintf('%s_%s',contact1,contact2)},size(temp_coherence_data,1),1)];
        event = [event;transpose(repelem(aligned_data.gait_events.Properties.VariableNames,n_events))];
        gait_cycle = [gait_cycle;repmat(transpose(1:n_events),length(aligned_data.gait_events.Properties.VariableNames),1)];
        visit = [visit;repelem(visit_number(i),size(temp_coherence_data,1),1)];
        
        if isfield(aligned_data,'stim_condition')
            stim_state = [stim_state;repmat({aligned_data.stim_condition},size(temp_coherence_data,1),1)];
        else % assume off stim
            stim_state = [stim_state;repmat({'OFF'},size(temp_coherence_data,1),1)];
        end
        
        if isfield(aligned_data,'med_condition')
            med_state = [med_state;repmat({aligned_data.med_condition},size(temp_coherence_data,1),1)];
        else % assume on med
            med_state = [med_state;repmat({'ON'},size(temp_coherence_data,1),1)];
        end
        
        if strcmp(power_type,'peri') || strcmp(power_type,'pre')
            time_range = [time_range;repelem(adj_time(i),size(temp_coherence_data,1),1)];
        end
        printStatus('');
    end
end
...
if remove_nan
    ...
    nan_inds = ismissing(data_coherence_mat);
    remove_inds = find(nan_inds(:,1));
    data_coherence_mat(remove_inds,:) = [];
...
data_coherence_table = array2table(data_coherence_mat,'VariableNames',col_names);

if strcmp(power_type,'peri') || strcmp(power_type,'pre')
    data_coherence_table = addvars(data_coherence_table,ids,visit,targets,med_state,stim_state,side,pair,event,gait_cycle,time_range,'Before',1,'NewVariableNames',{'Subject_ID','Visit','Stim_Target','Med_State','Stim_State','Side','Contact','Gait_Event','Gait_Cycle','Time_Range'});
else
    data_coherence_table = addvars(data_coherence_table,ids,visit,targets,med_state,stim_state,side,pair,event,gait_cycle,'Before',1,'NewVariableNames',{'Subject_ID','Visit','Stim_Target','Med_State','Stim_State','Side','Contact','Gait_Event','Gait_Cycle'});
end
...
if strcmp(power_type,'peri') || strcmp(power_type,'pre')
    data_phase_table = addvars(data_phase_table,ids,visit,targets,med_state,stim_state,side,pair,event,gait_cycle,time_range,'Before',1,'NewVariableNames',{'Subject_ID','Visit','Stim_Target','Med_State','Stim_State','Side','Contact','Gait_Event','Gait_Cycle','Time_Range'});
else
    data_phase_table = addvars(data_phase_table,ids,visit,targets,med_state,stim_state,side,pair,event,gait_cycle,'Before',1,'NewVariableNames',{'Subject_ID','Visit','Stim_Target','Med_State','Stim_State','Side','Contact','Gait_Event','Gait_Cycle'});
end
...
if export_data
    ...
    writetable(data_coherence_table,save_name);
    save(strrep(save_name,'.csv','.mat'),'data_coherence_table');
    ...
    save(strrep(save_name,'.csv','.mat'),'data_phase_table');
...
if freq_vals(2) < freq_vals(1)
    freq_bin_inds = abs(freq_bin_inds-length(freq_vals))+1;
