function lfp_data_noisierKey_ica_filt = removeConventionalStimArtifacts(aligned_data,side,varargin)
% Identifies and removes the adaptive stimulation artifacts using ICA and
% filtering
%
% INPUTS: Required
%
%       aligned_data[=]     The aligned datafile.
%
%               side[=]     Hemisphere of the LFP data
%                           Values: 0 for 'right', 1 for 'left'.
%
%         Optional
%
...
elseif side==0
    t_lfp = aligned_data.right_taxis;
    lfp_data_key0 = aligned_data.right_LFP_table.key0;
    lfp_data_key1 = aligned_data.right_LFP_table.key1;
    fs = aligned_data.DeviceSettings.Right.timeDomainSettings.samplingRate(end);
else
    error('Invalid side.');
end
