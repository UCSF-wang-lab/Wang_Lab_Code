function [data_power_table,data_phase_table] = extractDataFeaturesForML_Wide_Format(files,subject_ID,stim_target,visit_number,varargin)
...
%     3;...                           % gRCS01
%     3;3];                           % gRCS02
% extractDataFeaturesForML_Wide_Format(patient_files,patient_names,dbs_target,visit_number,'STFT_overlap',0.9);
% extractDataFeaturesForML_Wide_Format(patient_files,patient_names,dbs_target,visit_number,'power_type','peri','adj_time',0.05,'STFT_overlap',0.9);
% extractDataFeaturesForML_Wide_Format(patient_files,patient_names,dbs_target,visit_number,'power_type','peri','adj_time',0.1,'STFT_overlap',0.9);
% extractDataFeaturesForML_Wide_Format(patient_files,patient_names,dbs_target,visit_number,'power_type','pre','STFT_overlap',0.9);
...
%         adj_time(i) = calcAdjTime(aligned_data.gait_events,'inter-heel-strike');
    ...
    if isfield(signal_analysis_data,'Left')
        ...
        % DEBUG
%         col_names = createTableVariableNames(freq_bin_inds,freq_vals,signal_analysis_data.Left.Chan_Names);
        ...
        temp_power_data = nan(n_event_names*n_events,n_channels*n_freq_bins);
        temp_phase_data = nan(n_event_names*n_events,n_channels*n_freq_bins);
        for j = 1:n_channels
                ...
                for m = 1:n_events
                    if aligned_data.gait_events.(aligned_data.gait_events.Properties.VariableNames{k})(m) >= gait_event_range(1) && ...
                        ...
                        if ~isnan(event_time)
                            ...
                            temp_power_data((k-1)*n_events+m,(j-1)*n_freq_bins+1:j*n_freq_bins) = temp_power;
                            temp_phase_data((k-1)*n_events+m,(j-1)*n_freq_bins+1:j*n_freq_bins) = temp_phase;
                        end
                    end
                end
            end
    ...
    if isfield(signal_analysis_data,'Right')
        ...
        for j = 1:n_channels
            ...
            printStatus(sprintf('Completed extraction of Right %s',signal_analysis_data.Left.Chan_Names{j}));
        end
...
if strcmp(power_type,'peri') || strcmp(power_type,'pre')
    data_power_table = addvars(data_power_table,ids,visit,targets,med_state,stim_state,side,event,time_range,'Before',1,'NewVariableNames',{'Subject_ID','Visit','Stim_Target','Med_State','Stim_State','Side','Gait_Event','Time_Range'});
else
    data_power_table = addvars(data_power_table,ids,visit,targets,med_state,stim_state,side,event,'Before',1,'NewVariableNames',{'Subject_ID','Visit','Stim_Target','Med_State','Stim_State','Side','Gait_Event'});
end
...
if strcmp(power_type,'peri') || strcmp(power_type,'pre')
    data_phase_table = addvars(data_phase_table,ids,visit,targets,med_state,stim_state,side,event,time_range,'Before',1,'NewVariableNames',{'Subject_ID','Visit','Stim_Target','Med_State','Stim_State','Side','Gait_Event','Time_Range'});
else
    data_phase_table = addvars(data_phase_table,ids,visit,targets,med_state,stim_state,side,event,'Before',1,'NewVariableNames',{'Subject_ID','Visit','Stim_Target','Med_State','Stim_State','Side','Gait_Event'});
end
...
if export_data
    ...
    if strcmp(power_type,'instantaneous')
        save_name = fullfile(save_path,[datestr(datetime('today','Format','yyyy-MM-dd'),'yyyy-mm-dd'),'_',strrep(save_file,'.csv',['_',power_type,'_power_wide_format.csv'])]);
    elseif strcmp(power_type,'peri') || strcmp(power_type,'pre')
        if length(unique(adj_time)) == 1
            save_name = fullfile(save_path,[datestr(datetime('today','Format','yyyy-MM-dd'),'yyyy-mm-dd'),'_',strrep(save_file,'.csv',['_',power_type,'_',strrep(num2str(adj_time(1)),'.','__'),'_power_wide_format.csv'])]);
        else
            save_name = fullfile(save_path,[datestr(datetime('today','Format','yyyy-MM-dd'),'yyyy-mm-dd'),'_',strrep(save_file,'.csv',['_',power_type,'_power_wide_format.csv'])]);
        end
    end
...
%     save(strrep(save_name,'.csv','.mat'),'data_power_table');
...
function table_variable_names = createTableVariableNames(pair_inds,freq_vals,chan_names)
table_variable_names = cell(1,size(pair_inds,1)*length(chan_names));

splits = cellfun(@(x)strsplit(x,' '),chan_names,'UniformOutput',false);
extracted_channel_names = cellfun(@(x)x{1},splits,'UniformOutput',false);

positive_channel = cellfun(@(x)x(2:strfind(x,'-')-1),extracted_channel_names,'UniformOutput',false);
negative_channel = cellfun(@(x)x(strfind(x,'-')+1:end),extracted_channel_names,'UniformOutput',false);

% Check for consistency in channel names.
% Positive channel should be: 2, 3, 9, and 11
% Negative channel should be: 0, 1, 8, and 10
% If there is something flipped, reverse it because the positive and
% negative do not actually matter
correct_positive_channel = {'2','3','9','11'};
correct_negative_channel = {'0','1','8','10'};

positive_check = contains(positive_channel,{'2','3','9','11'});
negative_check = contains(negative_channel,{'0','1','8','10'});

if sum(positive_check) ~= 4 && sum(negative_check) ~= 4
    channel_check = ~and(positive_check,negative_check);
    n_changes = sum(channel_check);
    if  n_changes >= 1
        positive_channel{channel_check} = correct_positive_channel{channel_check};
        negative_channel{channel_check} = correct_negative_channel{channel_check};
    else
        error('Channels are not as expected. Exclude this patient or correct the recording channels.');
    end
end
...
for j = 1:length(extracted_channel_names)
    for i = 1:size(pair_inds,1)
        ...
        table_variable_names{(j-1)*size(pair_inds,1)+i} = [sprintf('p%in%i',str2num(positive_channel{j}),str2num(negative_channel{j})),'_f',var_name_start,'__',var_name_end];
    end
end
end
...
elseif strcmp(subjectID,'gRCS02')
    gait_event_range(1) = 0;
    gait_event_range(2) = max(aligned_data.gait_events{end,:});
elseif strcmp(subjectID,'gRCS03')
    gait_event_range(1) = 0;
    gait_event_range(2) = max(aligned_data.gait_events{end,:});
else
    gait_event_range(1) = 0;
