function mergeCoherenceGSLT_StepLenControl(varargin)
...
if ~exist('savePlot','var') || isempty(savePlot)
    ...
    plot_save_path = fullfile(parent_dir,'Figures','GSLT','Coherence_figs_adaptive_masked','Merged_StepLenControl',condition);
...
stepModfnames = cellfun(@(x) strrep(x,'step','step_mod'),cohdatafiles,'UniformOutput',false);
...
allStepMod.Left = cell(1,size(cohPairs,1));
allStepMod.Right = cell(1,size(cohPairs,1));
...
for i = 1:numel(cohdatafiles)
    ...
    load(fullfile(dataPath,stepModfnames{i}));
    ...
    if sum(step.Left{1},'all')~=0 % && sum(step.Right{1},'all')~=0
        ...
        allStepMod.Left = cellfun(@(x,y) cat(2,x,y),allStepMod.Left,step_mod.Left,'UniformOutput',false);
        allStepMod.Right = cellfun(@(x,y) cat(2,x,y),allStepMod.Right,step_mod.Right,'UniformOutput',false);
        ...
        clear step step_mod stepNums normalization toeOff lenNorm
    end
end
...
if saveData
    ...
    save(fullfile(dataPath,'allStepMod.mat'),'allStepMod','-v7.3');
