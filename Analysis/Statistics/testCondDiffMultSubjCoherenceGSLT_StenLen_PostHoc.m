function testCondDiffMultSubjCoherenceGSLT_StenLen_PostHoc(subjList,cond1,cond2,comparison,stepLen,varargin)
...
%
%                 stepLen[=]    'L' for long, 'M' for medium, 'S' for
%                               'short'
%
...
%           testCondDiffMultSubjCoherenceGSLT_StenLen_PostHoc([2,3,4],'ONmed_ONdbs_LeftAdaptive','ONmed_ONdbs_LeftMaladaptive','ONmed_ONdbs_AdaptvsMaladapt_Left','L','saveData',1,'savePlot',1,'unilat',1,'stat_test','ttest','dbs_cond','dbsopt');
%
% Date:     3/24/2024
...
if ~exist('saveData','var') || isempty(saveData)
    ...
    data_save_path = fullfile('X:\Patient Data\RC+S Data\gait_RCS aggregate data\Eleni\GSLT_coherence\Data',[comparison,'_',dbs_cond,'_',stat_test,'_',stepLen,'_subs_2_3_4_HigherFreq_upto70_v1']);
...
if ~exist('savePlot','var') || isempty(savePlot)
    ...
    plot_save_path = fullfile('X:\Patient Data\RC+S Data\gait_RCS aggregate data\Eleni\GSLT_coherence\Figures',[comparison,'_',dbs_cond,'_',stat_test,'_',stepLen,'_subs_2_3_4_HigherFreq_upto70_v1']);
    if ~exist(plot_save_path, 'dir')
       mkdir(plot_save_path);
    end
end

if strcmpi(stepLen,'L')
    stepModIndex = 1.2;
elseif strcmpi(stepLen,'M')
    stepModIndex = 1;
elseif strcmpi(stepLen,'S')
    stepModIndex = 0.8;
else
    disp('Invalid step length was given as input--check!');
end
...
for sub = 1:length(subjList)
    ...
    if ~contains(cond1,'Malada')
        stepMod = load(fullfile(data_path1, 'allStepMod.mat'));
    end
    ...
    if ~contains(cond2,'Malada')
        stepMod = load(fullfile(data_path2, 'allStepMod.mat'));
    end
    ...
    if unilat==0 || unilat==-1
        % Cond 1
        if ~contains(cond1,'Malada')
            step_mod = stepMod.allStepMod.Left{1};
            ind_sm = find(step_mod==stepModIndex);

            data_cond1.normCoh.Left = cellfun(@(x) x(:,:,ind_sm), data_cond1.normCoh.Left,'UniformOutput',false); 
            % allTO{1}.Left = cellfun(@(x) x(ind_sm), allTO{1}.Left,'UniformOutput',false); 

            clear step_mod ind_sm
        end
        ...
        if ~contains(cond2,'Malada')
            ...
            data_cond2.normCoh.Left = cellfun(@(x) x(:,:,ind_sm), data_cond2.normCoh.Left,'UniformOutput',false); 
            % allTO{2}.Left = cellfun(@(x) x(ind_sm), allTO{2}.Left,'UniformOutput',false); 
    ...
    if unilat==0 || unilat==1
        % Cond 1
        if ~contains(cond1,'Malada')
            step_mod = stepMod.allStepMod.Right{1};
            ind_sm = find(step_mod==stepModIndex);

            data_cond1.normCoh.Right = cellfun(@(x) x(:,:,ind_sm), data_cond1.normCoh.Right,'UniformOutput',false); 
            % allTO{1}.Right = cellfun(@(x) x(ind_sm), allTO{1}.Right,'UniformOutput',false); 
        ...
        if ~contains(cond2,'Malada')
            ...
            data_cond2.normCoh.Right = cellfun(@(x) x(:,:,ind_sm), data_cond2.normCoh.Right,'UniformOutput',false); 
            % allTO{2}.Right = cellfun(@(x) x(ind_sm), allTO{2}.Right,'UniformOutput',false); 
