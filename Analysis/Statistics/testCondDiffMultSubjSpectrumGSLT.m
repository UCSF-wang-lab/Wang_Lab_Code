function testCondDiffMultSubjSpectrumGSLT(subjList,cond1,cond2,comparison,varargin)
...
%           testCondDiffMultSubjSpectrumGSLT('keys',keys,'saveData',0,'savePlot',0,'comparison','adaptVSmaladapt_right','unilat',0);
%
% Date:     3/24/2024
...
if ~exist('savePlot','var') || isempty(savePlot)
    ...
    plot_save_path = fullfile('X:\Patient Data\RC+S Data\gait_RCS aggregate data\Eleni\GSLT_spectrum\Figures',[comparison,'_',dbs_cond,'_',stat_test,'_3_4_5_HigherFreq_upto70_v1']);
...
allsteps{1}.Left = cell(1,length(keys));
allsteps{1}.Right = cell(1,length(keys));
allsteps{2}.Left = cell(1,length(keys));
allsteps{2}.Right = cell(1,length(keys));
allTO{1}.Left = cell(1,length(keys));
allTO{1}.Right = cell(1,length(keys));
allTO{2}.Left = cell(1,length(keys));
allTO{2}.Right = cell(1,length(keys));
...
% Left
if unilat==0 || unilat==-1
    h_all_left = cell(1,length(allsteps{1}.Left));
    p_all_left = cell(1,length(allsteps{1}.Left));
        ...
        for f = 1:size(data_snip_cond1,1)
            for t = 1:size(data_snip_cond1,2)
                ...
                elseif strcmpi(stat_test,'lme')
                    spec_stacked = [squeeze(data_snip_cond1(f,t,:)); squeeze(data_snip_cond2(f,t,:))];
                ...
                clear h_new p lme spec_stacked
            end
...
% Right
if unilat==0 || unilat==1
    h_all_right = cell(1,length(allsteps{1}.Right));
    p_all_right = cell(1,length(allsteps{1}.Right));
    subjVar_stacked = [subjVar{1}; subjVar{2}];
    cond_stacked = [ones(length(subjVar{1}),1); 2*ones(length(subjVar{2}),1)];
